...
# Building
Prerequisites:  
1. Make sure you have Xcode iOS 17.0 - 17.1 installed
...
# Installing
There are two ways to install Fugu17 on your device: Via Safari or via USB

## Installing via Safari
To install Fugu17 via Safari, do the following (requires you to own a domain):  
...
## Installing via USB
1. Install `Fugu15_Developer.ipa`, e.g. via `ideviceinstaller -i Fugu15_Developer.ipa`. Alternatively, install Fugu17/Fugu17.ipa via TrollStore.
2. Open the newly installed "Developer" App (or whatever AppStore App you used) on your iPhone

# iDownload
Like all Fugu jailbreaks, Fugu17 ships with iDownload. The iDownload shell can be accessed on port 1337 (run `iproxy 1337 1337 &` and then `nc 127.1 1337` to connect to iDownload).  
...
Fugu17 also ships with the procursus bootstrap and Sileo. Run the `bootstrap` command in iDownload to install both. Afterwards, you might have to respring to force Sileo to show up on the Home Screen (`uicache -r`).
...
Q: Do you provide official support for Fugu17? Are any updates planned?  
A: No.  
